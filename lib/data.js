...
// Bepaal startcoördinaat voor een dag op basis van verblijf
export function getDayStartCoords(day) {
  if (!day) return MESSIRES_COORDS;
  if (day.stay === 'messires') return MESSIRES_COORDS;
  if (day.stay === 'clervaux') return CLERVAUX_COORDS;
  if (day.stay === 'transfer') return MESSIRES_COORDS;
  return MESSIRES_COORDS;
}

// Bepaal eindcoördinaat voor een dag
export function getDayEndCoords(day) {
  if (!day) return MESSIRES_COORDS;
  if (day.stay === 'transfer') return CLERVAUX_COORDS;
  return getDayStartCoords(day);
}
